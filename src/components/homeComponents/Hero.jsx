...
const Hero = () => {
  ...
  return (
    <section className='relative h-[87dvh] flex flex-col md:flex-row items-center justify-center gap-12 md:gap-4 p-10 md:p-12'>
      ...
      <div
        ref={scope}
        className='w-full md:max-w-[50%] relative text-center z-10'
      >
        <motion.div initial={{ y: 40, opacity: 0 }}>
          <h1 className='overflow-hidden text-white text-[2.2rem] sm:text-[3rem] font-mont font-bold'>
            {' '}
            Discover the Essence of Quality Baking{' '}
          </h1>
        </motion.div>
        <motion.div initial={{ y: 40, opacity: 0 }}>
          <h3 className='text-white text-base mx-auto font-poppins font-normal md:max-w-[80%] my-4'>
            From premium milk flavors to exquisite bakery materials, explore our
            NAFDAC-approved range crafted for perfection.
          </h3>
        </motion.div>
        <motion.div initial={{ y: 40, opacity: 0 }}>
          <div className='w-fit mx-auto'>
            <AboutButton
              action={handleRedirectionToShop}
              title='Learn More'
              arrow={true}
              color='white'
              border='none'
